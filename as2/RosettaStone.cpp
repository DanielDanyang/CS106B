...
Map<string, double> kGramsIn(const string& str, int kGramLength) {
    ...
    for (int i = 0; i <= str.size() - kGramLength; i++) {
        freq[str.substr(i, kGramLength)] += 1;
    }
    return freq;
}
...
Map<string, double> normalize(const Map<string, double>& input) {
    ...
    for (const double& i : input.values()) {
        squareSum += i * i;
    }
    double squareRootSum = sqrt(squareSum);

    if (squareRootSum == 0) {
        error("The map doesn’t contain at least one nonzero value");
    }

    Map<string, double> normalizedMap = input;
    for (const string& kgrams : normalizedMap.keys()) {
        normalizedMap[kgrams] /= squareRootSum;
    }
...
Map<string, double> topKGramsIn(const Map<string, double>& source, int numToKeep) {
    if (numToKeep < 0) {
        error("The numToKeep is invalid");
    }
    if (numToKeep > source.size()) {
        return source;
    }
    PriorityQueue<string> keepKgrams;
    for (const string& kgrams : source.keys()) {
        keepKgrams.enqueue(kgrams, source[kgrams]);
    }
    ...
    for (int i = 0; i < numToKeep; i++) {
        string kgrams = keepKgrams.dequeue();
        resultMap[kgrams] = source[kgrams];
    }
    return resultMap;
}
...
double cosineSimilarityOf(const Map<string, double>& lhs, const Map<string, double>& rhs) {
    ...
    for (const string& kgrams : lhs) {
        cosineSimilarity += lhs[kgrams] * rhs[kgrams];
    }
...
string guessLanguageOf(const Map<string, double>& textProfile,
    ...
    if (corpora.isEmpty()) error("The corpora is empty");
    PriorityQueue<string> maxSimilarity;
    maxSimilarity.enqueue("initialValue", 0);
    for (Corpus corpus : corpora) {
        double currentSimilarity = cosineSimilarityOf(textProfile, corpus.profile);
        if (currentSimilarity > -maxSimilarity.peekPriority()) {
            maxSimilarity.enqueue(corpus.name, -currentSimilarity);
        }
    }
    return maxSimilarity.peek();
}
