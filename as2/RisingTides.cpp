...
Grid<bool> floodedRegionsIn(const Grid<double>& terrain,
                            ...
                            double height) {
    Grid<bool> whetherFlooded(terrain.numRows(), terrain.numCols(), false);
    Queue<GridLocation> a;
    Set<GridLocation> checked;
    for (const GridLocation &gridLocation : sources) {
        checked.add(gridLocation);
        if (terrain.get(gridLocation) <= height) {
            a.enqueue(gridLocation);
            whetherFlooded.set(gridLocation, true);
        }
    }
    const Vector<pair<int, int>> directions = {{-1, 0}, {1, 0}, {0, -1}, {0, 1}};
    while (!a.isEmpty()) {
        GridLocation searchedPos = a.dequeue();
        for (const pair<int, int> &dir : directions) {
            GridLocation newPos(searchedPos.row + dir.first, searchedPos.col + dir.second);
            if (terrain.inBounds(newPos) && terrain.get(newPos) <= height && !checked.contains(newPos)) {
                checked.add(newPos);
                a.enqueue(newPos);
                whetherFlooded.set(newPos, true);
            }
        }
    }
    return whetherFlooded;
}
